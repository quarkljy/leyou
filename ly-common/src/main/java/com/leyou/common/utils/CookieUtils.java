    ...
    private static final String getDomainName(HttpServletRequest request) {
        ...
        if (serverName == null || serverName.equals("")) {
            ...
            if (serverName.indexOf(":") > 0) {
                ...
                String ipStr = ary[0];
	            //ip地址直接返回
	            boolean matches = ipStr.matches("^((25[0-5]|2[0-4]\\d|[01]?\\d\\d?).){3}(25[0-5]|2[0-4]\\d|[01]?\\d\\d?)$");
	            if (matches) {
		            return ipStr;
	            }
            }else {
	            boolean matches = serverName.matches("^((25[0-5]|2[0-4]\\d|[01]?\\d\\d?).){3}(25[0-5]|2[0-4]\\d|[01]?\\d\\d?)$");
	            if (matches) {
		            return serverName;
	            }
            }
